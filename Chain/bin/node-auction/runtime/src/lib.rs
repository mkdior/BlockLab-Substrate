...
/// Placeholder currency type for spending on auctions.
pub type Currency = ();
// End Auction related //
...
impl AuctionHandler<AccountId, Balance, BlockNumber, AuctionId> for Handler {
    fn on_new_bid(
        ...
        if let Some(_bid) = last_bid_ {
            ...
            //println!("First bid on auction [{:#?}]", id_);
        }
        OnNewBidResult {
            accept_bid: true,
            auction_end: None,
...
impl_runtime_apis! {
    ...
    impl auction_api::AuctionInformationAPI<Block, AccountId, AuctionId, Balance, BlockNumber, GeneralInformationContainer> for Runtime {
        ...
        fn auction_query_formal(
            id: AuctionId,
        ) -> Option<UIAuctionInfo<AccountId, BlockNumber, GeneralInformationContainer>> {
